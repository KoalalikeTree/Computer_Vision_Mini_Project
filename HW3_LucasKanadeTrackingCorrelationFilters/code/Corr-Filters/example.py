...
print("dx is form ", dx.min(), "to", dx.max())
dy = np.arange(-np.floor(dsize[0]/2), np.floor(dsize[0]/2)+1, dtype=int)
print("dy is form ", dy.min(), "to", dy.max())
[dpx, dpy] = np.meshgrid(dx, dy)
dpx = dpx.reshape(-1, 1)
print("dpx.shape", dpx.shape)
dpy = dpy.reshape(-1, 1)
print("dpy.shape", dpy.shape)
...
print("all_patches.shape = (N * h, w)", all_patches.shape)
...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        xn = imwarp(dp[i, :] + gnd_p)  # (29, 45)
        X[:, i] = xn.reshape(-1)   # (1, 1305)
        Y[i] = np.exp(-np.dot(dp[i, :], dp[i, :])/sigma)  # scalar
        all_patches[(i*dsize[0]):((i+1)*dsize[0]), :] = xn
        # print(i*dsize[0], (i+1)*dsize[0])
        cropax.set_data(xn)
        ...
        print(g)
        new_figure1 = plt.figure()
        img_corre = scipy.ndimage.correlate(img, g)
        plt.imshow(img_corre, cmap='Greys')
        new_figure2 = plt.figure()
        g_conv = g[::-1, ::-1]
        img_convol = scipy.ndimage.convolve(img, g_conv)
        plt.imshow(img_convol, cmap='Greys')
        plt.show()

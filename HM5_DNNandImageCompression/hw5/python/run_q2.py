...
for itr in range(max_iters):
    ...
    for xb, yb in batches:
        ...
        output_layer = forward(layer1, params, name='output', activation=softmax)

        # loss
        # be sure to add loss and accuracy to epoch totals
        loss, acc = compute_loss_and_acc(yb, output_layer)
        # print(loss, acc)
        total_loss += loss
        avg_acc += acc

        # backward
        delta_output = output_layer - yb
...
for k, v in params.items():
    """
    # params:
    Wlayer1 grad_Wlayer1: (input_layer_node, layer1_node)
    blayer1 grad_blayer1：(layer1_node, )
    Woutput grad_Woutput: (layer1_node, output_layer_node)
    boutput grad_boutput: (output_layer_node, )
    cache_layer1 include  (X, pre_act, post_act)
        X                 (examples, input_layer_node)
        pre_act           (examples, layer1_node)
        post_act          (examples, layer1_node)
    cache_output include  (X, pre_act, post_act)
        X                 (examples, layer1_node)
        pre_act           (examples, output_layer_node)
        post_act          (examples, output_layer_node)
    """

    if '_' in k:
        # this is a gradient and doesn't need any change
        continue
    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                params_a[k][i,j] = v[i,j] + eps
                layer1 = forward(xb, params_a, 'layer1')
                probs = forward(layer1, params_a, 'output', softmax)
                loss_a, acc_a = compute_loss_and_acc(yb, probs)

                params_b = copy.deepcopy(params)
                params_b[k][i, j] = v[i, j] - eps
                layer1 = forward(xb, params_b, 'layer1')
                probs = forward(layer1, params_b, 'output', softmax)
                loss_b, acc_b = compute_loss_and_acc(yb, probs)

                params['grad_'+k][i, j] = (loss_a -loss_b) / (2 * eps)


    if 'b' in k:
        for i in range(v.shape[0]):
            params_a = copy.deepcopy(params)
            params_a[k][i] = v[i] + eps
            layer1 = forward(xb, params_a, 'layer1')
            probs = forward(layer1, params_a, 'output', softmax)
            loss_a, acc_a = compute_loss_and_acc(yb, probs)

            params_b = copy.deepcopy(params)
            params_b[k][i] = v[i] - eps
            layer1 = forward(xb, params_b, 'layer1')
            probs = forward(layer1, params_b, 'output', softmax)
            loss_b, acc_b = compute_loss_and_acc(yb, probs)

            params['grad_' + k][i] = (loss_a - loss_b) / (2 * eps)
